...
The real-install jobs exercise a fresh privacy-first installation, simulated upgrades from a pre-1.1.0 database with and without an explicit consent decision, incomplete and unsafe configuration, immediate and consent-controlled rendering, the 30-second wait cap, default/website/store inheritance, frontend and admin block resolution, callback-compatible loader delivery, and disabled-mode suppression. Platform versions are deliberately pinned so upstream releases cannot silently change the test baseline; updates should be made explicitly after local validation.
