...
| `BasicRum_Analytics_Helper_Data` | Retrieves admin config values: `isEnabled()`, `isOptInRequired()`, `getBeaconEndpoint()`. |
...
| `boomerang-loader-v15.js` | Standard async loader |
| `opt-in-boomerang-loader-v1-15.js` | GDPR-compliant loader (requires user consent) |
...
3. **Page Type Detection**: The `p_type` variable in `Loader.php` is currently hardcoded to `"test"`. This should be extended to detect actual page types (category, product, cms, checkout, etc.).
